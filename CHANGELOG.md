...
- **Dead `BATCH_NOTIFICATION_THRESHOLD` constant** – `notifications.ts` defined `BATCH_NOTIFICATION_THRESHOLD = 2` but `flushPendingNotifications` hard-coded `=== 1` directly, making the constant a no-op. The condition now uses the constant so the batching threshold is controlled from one place
